...
test("LoginForm renders error messages", async () => {
  ...
  const screen = await page.render(<Stub initialEntries={["/login"]} />);

  await screen.getByRole("button", { name: "Login" }).click();

  await expect.element(screen.getByText(USER_MESSAGE)).toBeVisible();
  await expect.element(screen.getByText(PASSWORD_MESSAGE)).toBeVisible();
});
```
